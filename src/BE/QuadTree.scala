...
/*
  private def scaleCoords(valor: Double, cords: Coords): Coords = {
    new Coords((cords._1._1 * valor.toInt, cords._1._2 * valor.toInt),
      (((cords._2._1 + 1) * valor).toInt - 1, ((cords._2._2 + 1) * valor).toInt - 1))
  }

  private def scaleAxis(valor: Double, axis: Int): Int = {
    ((axis+1)*valor).toInt-1
  }

  private def unscaleCoords(valor: Double, cords: Coords): Coords = {
    new Coords((cords._1._1 / valor.toInt, cords._1._2 / valor.toInt),
      (((cords._2._1 + 1) / valor).toInt - 1, ((cords._2._2 + 1) / valor).toInt - 1))
  }

  private def isLeafable(cords:Coords):Boolean={
    val x1 = cords._1._1
    val y1 = cords._1._2
    val x2 = cords._2._1
    val y2 = cords._2._2
    if(x2-x1!=0 || y2-y1!=0) false
    else true
    //if(x2-x1==0 && y2-y1==0) true
    //else false
  }

  private def isEmptyable(cords:Coords):Boolean={
    val x1 = cords._1._1
    val y1 = cords._1._2
    val x2 = cords._2._1
    val y2 = cords._2._2
  ...
  private def isInQuadrant(scaledColor:Coords,cords:Coords):Boolean={
    if(scaledColor._1._1<cords._1._1 || scaledColor._1._2<cords._1._2 || scaledColor._2._1>cords._2._1 || scaledColor._2._2>cords._2._2) false
    else true
  }

  private def calculateAverageColor(color1:Color,color2:Color):Color={
    val r = (color1.getRed+color2.getRed)/2
    val g = (color1.getGreen+color2.getGreen)/2
    val b = (color1.getBlue+color2.getBlue)/2
    new Color(r,g,b)
  }

  private def findAverageColor[A](scaledCords:Coords,qt:QTree[A]):Color={
    qt match {
      case QLeaf((_, color: Int)) => HCtoColor(color)
      case QNode(cords:Coords, one, two, three, four) =>
        val x1 = cords._1._1
        val y1 = cords._1._2
        val x2 = cords._2._1
        val y2 = cords._2._2
        val comprimento = x2-x1+1
        val altura = y2-y1+1
        val cordsOfOne:Coords = ((x1, y1),(x2 - math.ceil(comprimento / 2f).toInt, y2 - math.ceil(altura / 2f).toInt))
        val cordsOfTwo:Coords = ((x1 + comprimento / 2, y1), (x2, y2 - math.ceil(altura / 2f).toInt))
        val cordsOfThree:Coords = ((x1, y1 + altura / 2), (x2 - math.ceil(comprimento / 2f).toInt, y2))
        val cordsOfFour:Coords = ((x1 + comprimento / 2, y1 + altura / 2), (x2, y2))
        val booleans:List[Boolean] = List(isInQuadrant(scaledCords,cordsOfOne),isInQuadrant(scaledCords,cordsOfTwo),
                                          isInQuadrant(scaledCords,cordsOfThree),isInQuadrant(scaledCords,cordsOfFour))
    }
  }

  def makeLeafs[A](valor: Double, qt: QTree[A]): QTree[Coords] = {

    def aux(x1: Int, y1: Int, x2: Int, y2: Int): QTree[Coords] = {
      val cords = ((x1,y1),(x2,y2))
      if(isEmptyable(cords))
        QEmpty
      else if(isLeafable(cords))
        QLeaf(cords,averageColor(unscaleCoords(valor,cords),qt))
      else{
        val comprimento = x2-x1+1
        val altura = y2-y1+1
        QNode(((x1,y1),(x2,y2)):Coords,aux(x1, y1, x2 - math.ceil(comprimento / 2f).toInt, y2 - math.ceil(altura / 2f).toInt),
                                       aux(x1 + comprimento / 2, y1, x2, y2 - math.ceil(altura / 2f).toInt),
                                       aux(x1, y1 + altura / 2, x2 - math.ceil(comprimento / 2f).toInt, y2),
                                       aux(x1 + comprimento / 2, y1 + altura / 2, x2, y2))
      }
    }
        aux(0*valor.toInt, 0*valor.toInt, scaleAxis(valor,getCompQT(qt)), scaleAxis(valor,getAltQT(qt)))
  }
*/
